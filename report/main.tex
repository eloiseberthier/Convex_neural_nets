...
We consider single hidden-layer neural networks. It defines a class of prediction functions on $\mathbb{R}^d$ parametrized as: $f(x) =\sum_{j=1}^k \eta_j \sigma(w_j^T x + b_j)$, where $\sigma$ is a fixed activation function, $k$ is the number of hidden units, and the $(\eta_j)_{j=1,...,k}$ are the weights of the output layer.\\
...
For a fixed value of $k$, training a neural network is an empirical risk minimization problem: $\min_{\eta, w, b} \frac{1}{n} \sum_{i=1}^n \ell(f(x_i), y_i) + \Omega(\eta, w, b)$, where $\ell$ is a smooth convex loss function and $\Omega$ a convex regularization function.
